                ...
                if df.empty:
                    ...
                    if run_price > 0 and stop_price > 0:
                        ...
                        if res > self.Rc and res > ExpReturn:
                            labels.append([ismcode, 1.0, 0.0, 0.0, 0.0]) 
                        elif res > self.Rc and res <= ExpReturn:
                            labels.append([ismcode, 0.0, 1.0, 0.0, 0.0]) 
                        elif res <= self.Rc and res > ExpReturn:
                            labels.append([ismcode, 0.0, 0.0, 1.0, 0.0]) 
                        elif res <= self.Rc and res <= ExpReturn:
                            labels.append([ismcode, 0.0, 0.0, 0.0, 1.0]) 
                    else:
                        continue        
            labels = pd.DataFrame(labels, columns=['ismcode', 'C1', 'C2', 'C3', 'C4'])                           
            conn.close()
        # test mode
        else:
            # iterate on all ism packages
            ...
            for _i, ism_pack in enumerate(data):
                ...
                SecuCode_ls = flatten(list(map(lambda x: [y[1] for y in x[1:]], ism_pack[1:])))
                PriorType_ls = flatten(list(map(lambda x: [y[2] for y in x[1:]], ism_pack[1:])))
                labels.append([ismcode, 0.0, 0.0, 0.0, 0.0])
            labels = pd.DataFrame(labels, columns=['ismcode', 'C1', 'C2', 'C3', 'C4'])   
...
class ism_data(object):
    ...
    def next_batch(self):
        ...
        label_batch = sample[['C1','C2','C3','C4']].values
        sample = sample.drop(columns=['ismcode','C1','C2','C3','C4'])
        input_batch = sample.values
        return input_batch, label_batch
    
    def validation_set(self):
        labels = self.data[['C1','C2','C3','C4']].values
        inputs = self.data.drop(columns=['ismcode','C1','C2','C3','C4'])
