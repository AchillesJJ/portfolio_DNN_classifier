...
class ism_data(object):
    ...
    def __init__(self, ism_path, interval=7, Rc=0.0, batch_size=None, mode=0, random_seed=123):
            ...
            if len(data): # not empty
                ...
                for _i, ism_pack in enumerate(data):
                    ...
                    if df.empty:
                        ...
                        if res > self.Rc:
                            labels.append([ismcode, 1.0, 0.0] + sparse_ls) 
                        else:
                            labels.append([ismcode, 0.0, 1.0] + sparse_ls) 
        ...
        # test mode
            ...
            if len(data):
                ...
                for _i, ism_pack in enumerate(data):
                    ...
                    for i in ls:
                        sparse_ls.append(self.SF_ls[self.SF_ls['SecuCode']==i].index.tolist()[0])
                    labels.append([ismcode, 0.0, 0.0] + sparse_ls)
    ...
    def sparse_encoding(self):
        ...
        sparse_encode = [x for x in sparse_encode if x != []]
    ...
    def dense_encoding(self, sparse_code_ls):
        ...
        for sparse_code in sparse_code_ls:
            ...
            for i in sparse_code[3:]:
                dense_code[int(i)] = 1.0
            # dense_code = dense_code
            input_batch.append(dense_code)
            label_batch.append(sparse_code[1:3])
            ismcode.append([sparse_code[0]])
        return input_batch, label_batch, ismcode
    ...
    def next_batch(self, batch_size=None):
        ...
        if batch_size is None:
            ...
            sparse_batch = random.sample(self.sparse_code, batch_size)
        # sparse to dense code
        input_batch, label_batch, _ = self.dense_encoding(sparse_batch)
    ...
    def validation_set(self):
        ...
        """
        input_all, label_all, _ = self.dense_encoding(self.sparse_code)
    ...
    def evaluation_set(self):
        ...
        """
        input_all, _, ismcode = self.dense_encoding(self.sparse_code)
        return input_all, ismcode
