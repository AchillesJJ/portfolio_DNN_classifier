...
class ism_list(object):
        ...
        if self.mode == 0 or self.mode == 1:
            ...
            if len(data): # not empty
                ...
                for _i, ism_pack in enumerate(data):
                    ...
                    flag_run = "SELECT InnerCode, ClosePrice FROM QT_DailyQuote WHERE " + \
                            "InnerCode IN (" + ",".join(InnerCode_ls) + ") AND " + \
                            "TradingDay=\'" + ProRunDate + "\'"
                    flag_stop = "SELECT InnerCode, ClosePrice FROM QT_DailyQuote WHERE " + \
                            "InnerCode IN (" + ",".join(InnerCode_ls) + ") AND " + \
                    ...
                    if df.empty:
                        ...
                        res = ((stop_price-run_price)/run_price)*250/interval # 250 market days/year
                        # sparse_encoding
                        sparse_ls = []
                        ...
                        if res > self.Rc:
                            logit_labels.append([ismcode, 1.0, 0.0] + sparse_ls)
                        else:
                            logit_labels.append([ismcode, 0.0, 1.0] + sparse_ls)
        ...
        # testing mode
            ...
            if len(data):
                ...
                for _i, ism_pack in enumerate(data):
                    ...
                    for i in SecuCode_ls:
                        sparse_ls.append(self.SF_ls[self.SF_ls['SecuCode']==i].index.tolist()[0])
                    logit_labels.append([ismcode, 0.0, 0.0] + sparse_ls)
        logit_labels = [x for x in logit_labels if x != []]
    ...
    def dense_encoding(self, logit_labels_ls):
        ...
        for logit_labels in logit_labels_ls:
            ...
            for i in logit_labels[3:]:
                dense_code[int(i)] = 1.0
            input_batch.append(dense_code)
            label_batch.append(logit_labels[1:3])
...
class ism_data(object):
    ...
    def __init__(self, ism_path, batch_size, interval, scaler_ls=None, Rc=0.0, mode=0):
        ...
        print("logit num is {}, factor num is {}".format(len(self.data_logit_labels), len(self.data_factor)))
    ...
    def next_batch(self, batch_size=None):
        ...
        if batch_size is None:
            ...
            sample_logit_labels = random.sample(self.data_logit_labels, batch_size)
        # collect index
        ...
        logit_batch, label_batch = self.ism_list.dense_encoding(sample_logit_labels)
        
        return logit_batch, factor_batch, label_batch
    ...
    def validation_set(self):
        ...
        all_logit, all_label = self.ism_list.dense_encoding(all_logit_and_labels)
        
        return all_logit, all_factor, all_label
        
    def evaluation_set(self):
        all_logit_and_labels = self.data_logit_labels
        index = list(map(lambda x: x[0], all_logit_and_labels))
        # reorder factor list according to ism index
        all_factor = self.data_factor[self.data_factor['ismcode'].isin(index)].copy()
        all_factor['ismcode'] = all_factor['ismcode'].astype('category')
        all_factor['ismcode'].cat.reorder_categories(index, inplace=True)
        all_factor.sort_values('ismcode', inplace=True)
        all_factor = all_factor.drop(columns='ismcode').values
        all_logit, _ = self.ism_list.dense_encoding(all_logit_and_labels)
        all_ismcode = np.asarray(index)
        all_ismcode = np.reshape(all_ismcode, (-1, 1))
        
        return all_logit, all_factor, all_ismcode
